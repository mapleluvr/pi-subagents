...
• Run-time policy: omit timeoutMs/maxRuntimeMs, turnBudget, and toolBudget by default. Set timeoutMs/maxRuntimeMs only when the user explicitly requires a hard deadline; set turnBudget/toolBudget only when the user explicitly requires that specific limit. Do not substitute one finite limit for another. A requested launch timeout is shared across the entire single, parallel, or chain run and terminates active child work when it expires.
• Model-route policy: omit every per-run model field unless the user explicitly requests that exact model. The configured primary and fallback models are the pre-authorized route; provider/model failures may use that route automatically. A :thinking suffix is covered by model override permission. Interactive ask may confirm once; headless ask rejects. A child tool failure is not a provider failure: do not retry with a different model; recover the same run/session or inspect artifacts.
...
• Run-time policy: omit timeoutMs/maxRuntimeMs, turnBudget, and toolBudget by default. Pass timeoutMs/maxRuntimeMs only when the user explicitly requires a hard deadline; pass turnBudget/toolBudget only when the user explicitly requires that specific limit. Do not substitute one finite limit for another. A requested launch timeout is one run-level deadline shared by foreground and async/background runs.
...
• context can be "fresh" or "fork"; omitted uses each agent defaultContext, otherwise fresh. Omit timeoutMs/maxRuntimeMs, turnBudget, and toolBudget by default. Set timeoutMs/maxRuntimeMs only for a hard deadline the user explicitly requires; set turnBudget/toolBudget only when the user explicitly requires the corresponding limit. Do not substitute one finite limit for another. A requested timeout applies to the entire foreground or async/background run.
• Omit every per-run model field unless the user explicitly requests that exact model. The configured primary and fallback models are the pre-authorized route. A :thinking suffix is covered by model override permission; interactive ask may confirm once and headless ask rejects. A child tool failure is not a provider failure: do not retry with a different model; recover the same run/session or inspect artifacts.
